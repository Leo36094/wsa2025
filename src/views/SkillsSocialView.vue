...
import { SocialIconEnum } from '@/components/icons/enum'
...
const skillI18n = computed(() =>
  Object.entries(SocialIconEnum).reduce((acc: Record<string, string>, [key, value]) => {
    acc[value] = t(`skills.social_${key}`)
    return acc
  }, {}),
)
const skillList = computed(() =>
  Object.entries(SocialIconEnum).map(([key, value]) => ({
...
const displayIcon = computed(() => selectedSkill.value.icon as SocialIconEnum)

const copyText = computed(() => {
  const copy = t(`skills.social_${selectedSkill.value.id}_copy`)
  const title = t(`skills.social_${selectedSkill.value.id}`)
