  ...
  <div class="skills-manufacturing">
    <div class="skills-manufacturing-container">
      <h2 class="skills-manufacturing-title" data-aos="fade-down">
        {{ $t('skills.category_construction') }}
      </h2>
        ...
        <div class="gallery-container">
          ...
          <div class="gallery-copy" data-aos="fade-down">
            <div class="icon-button">
              <IconConstruction :name="displayIcon" />
            </div>
...
import { ConstructionIconEnum } from '@/components/icons/enum'
...
const skillI18n = computed(() =>
  Object.entries(ConstructionIconEnum).reduce((acc: Record<string, string>, [key, value]) => {
    acc[value] = t(`skills.construction_${key}`)
    return acc
  }, {}),
)
const skillList = computed(() =>
  Object.entries(ConstructionIconEnum).map(([key, value]) => ({
...
const displayIcon = computed(() => selectedSkill.value.icon as ConstructionIconEnum)

const copyText = computed(() => {
  const copy = t(`skills.construction_${selectedSkill.value.id}_copy`)
  const title = t(`skills.construction_${selectedSkill.value.id}`)
